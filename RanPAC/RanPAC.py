...
class Learner(BaseLearner):
    def __init__(self, args):
        super().__init__(args)
        # [Debug-BP9 模型构造分支]
        # 重点看: args['model_name'], args['convnet_type']
        # 面试点: PETL 方法(adapter/ssf/vpt)与骨干网络后缀要一致，否则报错。
    ...
    def replace_fc(self,trainloader):
        # [Debug-BP10 Phase-2 核心: 分类头更新]
        # RanPAC 核心在这里: 用特征统计量更新分类器权重，而不是长期反向传播全网络。
        # 重点看: Features_f, Y, self.Q, self.G, Wo
        ...
        if self.args['use_RP']:
            # [Debug-BP11 随机投影路径]
            # 论文对应: h(x)=ReLU(f(x)W_rand)，然后解带岭回归闭式解。
            # 重点看维度:
            # Features_f: [N, D], W_rand: [D, M], Features_h: [N, M], Y: [N, C]
    ...
    def optimise_ridge_parameter(self,Features,Y):
        # [Debug-BP13 岭参数搜索]
        # 重点看: ridges, losses, 返回 ridge
        # 面试点: 为什么需要 ridge? 缓解 G 矩阵病态/过拟合。
    ...
    def incremental_train(self, data_manager):
        # [Debug-BP14 增量任务入口]
        # 调用路径关键枢纽: 任务状态更新 -> DataLoader 构建 -> self._train
        # 重点看: _cur_task, _known_classes, _classes_seen_so_far, class_increments
    ...
    def _train(self, train_loader, test_loader, train_loader_for_CPs):
        # [Debug-BP15 训练策略分发]
        # 分两段: Phase-1(首任务 PETL 微调，可选) + Phase-2(每任务 replace_fc)
        # 重点看: self._cur_task, args['model_name'], args['use_RP']
    ...
    def setup_RP(self):
        # [Debug-BP17 RP 参数准备]
        # 论文对应随机投影模块: 固定随机矩阵 W_rand，不参与训练。
    ...
    def _init_train(self, train_loader, test_loader, optimizer, scheduler):
        # [Debug-BP18 首任务 SGD/PETL 训练循环]
        # 重点看: logits, loss, preds, train_acc, test_acc
        # 面试点: 这里是标准 PyTorch 训练三步 (zero_grad -> backward -> step)。
        # prog_bar = tqdm(range(self.args['tuned_epoch']))
        # 临时修改
        prog_bar = tqdm(range(2))
