...
class CosineLinear(nn.Module):
    ...
    def forward(self, input):
        # [Debug-BP19 分类头前向]
        # use_RP=False: 余弦分类器
        # use_RP=True : RanPAC 的 RP 特征 + 线性层
        # 重点看: input.shape, self.weight.shape, out.shape
        if not self.use_RP:
...
def get_convnet(args, pretrained=False):
    ...
    # [Debug-BP20 骨干网络工厂函数]
    # 面试点: 为什么用工厂函数? 统一不同 backbone/PETL 的实例化入口。
    # 重点看: args['convnet_type'] 映射到哪一类模型。
...
class BaseNet(nn.Module):
    ...
    def forward(self, x):
        # [Debug-BP21 Backbone -> Head 数据流]
        # 调用路径: Learner._init_train/_eval_cnn -> self._network(inputs) -> BaseNet.forward
        # 重点看: x['features'] 与 logits 的对应关系。
        x = self.convnet(x)
...
class ResNetCosineIncrementalNet(BaseNet):
    ...
    def update_fc(self, nb_classes):
        # [Debug-BP22 增量扩展分类头]
        # 每个任务可能类别数增加，旧权重复制到新 fc，新增类别权重置零。
        # 重点看: nb_output, nb_classes, weight 拼接形状。
