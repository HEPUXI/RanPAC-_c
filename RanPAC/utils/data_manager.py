...
class DataManager(object):
    def __init__(self, dataset_name, shuffle, seed, init_cls, increment,use_input_norm=False):
        # [Debug-BP24 数据管理器初始化]
        # 重点看: dataset_name, _class_order, _increments
        # 面试点: 增量学习中“每个任务包含哪些类”由 _increments 定义。
    ...
    def get_dataset(self, indices, source, mode, appendent=None, ret_data=False):
        # [Debug-BP25 构造任务数据集]
        # 重点看: indices(当前任务类别范围), mode(train/test), 返回的 DummyDataset 长度
        if source == "train":
...
class DummyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # [Debug-BP26 DataLoader 单样本输出]
        # 返回 (idx, image_tensor, label)
        # 面试常问: 为什么 batch 解包时写成 (_, inputs, targets) ?
        if self.use_path:
