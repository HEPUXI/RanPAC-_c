...
def train(args):
    # [Debug-BP3 多随机种子入口]
    # 重点看: seed_list, device
    # 面试点: 为什么先深拷贝? 避免后续修改 args 时影响原始配置。
...
def _train(args):
    ...
    # [Debug-BP4 任务切分配置]
    # 重点看: init_cls, increment
    # CIL 中 init_cls 是初始类别数，increment 是每个增量任务新增类别数。
    ...
    # [Debug-BP5 Learner 实例化]
    # 重点看: model._network 的具体类型（ViT/ResNet），由 args 决定。
    # 调用路径: trainer._train -> Learner.__init__ -> get_convnet -> timm.create_model
    model = Learner(args)
    ...
    if args['dataset']=='core50':
        ...
        #cil datasets
        model.is_dil=False
        # [Debug-BP6 DataManager 实例化]
        # 重点看: data_manager._increments, data_manager._class_order
        # 面试点: 类别顺序是否打乱、每个 task 的类别区间如何生成。
    ...
    # 当前代码仅跑前2个任务用于快速调试，面试时要知道原始逻辑是遍历全部任务。
    for task in range(2):
        if model.is_dil:
        ...
        # [Debug-BP7 单任务主调用]
        # 调用路径: incremental_train -> _train -> (可选 PETL SGD) -> replace_fc
        # 重点看: model._cur_task, _known_classes, _classes_seen_so_far
        model.incremental_train(data_manager)

        # [Debug-BP8 评估调用]
        # 调用路径: eval_task -> _eval_cnn -> _evaluate -> accuracy
        # 重点看: predicted_classes, true_classes, acc_grouped
